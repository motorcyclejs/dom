...
import { svgh } from './svg';

export interface SvgHyperscriptHelperFn<T extends Element> {
  (selector: string, data: VNodeData, children: Array<VNode<any> | null>): VNode<T>;
  (data: VNodeData, children: Array<VNode<any> | null>): VNode<T>;
}

function hh <T extends SVGElement> (tagName: SvgTagNames): SvgHyperscriptHelperFn<T> {
  return function helper (selector: string, data: VNodeData, children: Array<VNode<any> | null>) {
    return isSelector(selector)
      ? svgh<T>(tagName + selector, data, children)
      : svgh<T>(tagName, selector, data as Array<VNode<any> | null>);
  } as SvgHyperscriptHelperFn<T>;
}
